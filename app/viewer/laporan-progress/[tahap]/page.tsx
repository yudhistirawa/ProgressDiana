...
type Props = { params: { tahap: string }; searchParams?: { project?: string } };
  ...
  return (
    <div className="relative min-h-screen w-full bg-neutral-50 text-neutral-900">
      ...
      <header className="sticky top-0 z-20 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 border-b border-neutral-200 shadow-sm">
        <div className="mx-auto max-w-7xl px-4 py-3 grid grid-cols-12 items-center gap-3">
          <div className="col-span-6 flex items-center gap-2">
            <Link href={`/viewer/laporan-progress?project=${projectParam}`} className="inline-flex items-center justify-center h-9 w-9 rounded-full border border-neutral-300 bg-white text-neutral-700 hover:bg-neutral-100 shadow-sm" title="Kembali">
            ...
            <h1 className="text-lg font-semibold text-neutral-900">Detail Laporan Tahap {stage}</h1>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <StageReportListClient stage={stage} readOnly={true} project={projectParam} />
      </main>
    </div>
  );
}
