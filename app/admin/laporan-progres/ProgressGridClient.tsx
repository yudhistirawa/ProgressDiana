    ...
    (async () => {
      try {
        ...
        const items: StageItem[] = Array.isArray(list)
          ? list.map((s: any, i: number) => ({ id: i + 1, stageId: s?.id, title: s?.name || `Tahap ${i + 1}` }))
          : [];
        setStages(items);
      } catch {
        setStages([]);
    ...
    <div className="space-y-4">
      ...
      <section className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
        {filtered.map((stage) => (
          <Link
            key={stage.id}
            href={`/admin/laporan-progres/${stage.id}?project=${project}${stage.stageId != null ? `&stageId=${stage.stageId}` : ""}`}
