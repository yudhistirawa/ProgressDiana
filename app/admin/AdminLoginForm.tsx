      ...
      <div className="rounded-3xl ring-1 ring-neutral-200 bg-white/90 shadow-xl backdrop-blur p-6">
        ...
        <form
          ...
          if (!res.ok) {
            ...
            if (msg.includes('user not found') || msg.includes('admin tidak ditemukan') || msg.includes('no user record')) {
    ...
    <div className="rounded-3xl ring-1 ring-neutral-200 bg-white/90 shadow-xl backdrop-blur p-8">
      ...
      <form
        ...
        onSubmit={async (e) => {
          ...
          if (!res.ok) {
            const msg = res.message.toLowerCase();
            if (msg.includes('user not found') || msg.includes('admin tidak ditemukan') || msg.includes('no user record')) {
              showAlert('error', 'Username Tidak Ditemukan', `Username "${username}" tidak terdaftar di sistem.\n\nSilakan hubungi administrator untuk registrasi akun.`);
            } else if (msg.includes('wrong-password') || msg.includes('password is invalid') || msg.includes('invalid password')) {
              showAlert('error', 'Password Salah', `Password yang Anda masukkan untuk username "${username}" tidak sesuai.\n\nPastikan password diketik dengan benar.`);
            } else if (msg.includes('too many requests') || msg.includes('blocked')) {
              showAlert('warning', 'Terlalu Banyak Percobaan', 'Sistem mendeteksi terlalu banyak percobaan login.\n\nHarap tunggu beberapa menit sebelum mencoba lagi.');
            } else {
              showAlert('error', 'Login Gagal', res.message || 'Terjadi kesalahan sistem.');
            }
            return;
          }
          setIsLoading(true);
      ...
      {alertState.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          {/* Backdrop */}
          <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px]" onClick={closeAlert} />

          {/* Alert Card */}
          <div className="relative w-full max-w-sm animate-in slide-in-from-top-5 duration-500">
            <div className="bg-white rounded-2xl shadow-2xl ring-1 ring-neutral-200 overflow-hidden">
              {/* Header */}
