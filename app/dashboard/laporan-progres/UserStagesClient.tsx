...
export default function UserStagesClient({ project = "diana" }: { project?: ProjectKey }) {
  ...
  const items = useMemo(() => {
    return stages.map((s, i) => {
      return {
        id: i + 1, // nomor urut untuk routing statis
        stageId: s.id, // ID asli dari config untuk identitas stabil
        title: s.name || `Laporan ${i + 1}`,
      } as { id: number; stageId?: number; title: string };
    });
  }, [stages]);
